...
//V11.9 - Added support for Schrader A9052102 used on Mercedes AMG GT (A0009052102). A big thanks to Rm for helping out on this one. 
//        This uses the CC1101 FIFO because the bit rate is too high for ProMicro but should be better using the FIFO
//        Changed handling of CC1101 calibration to support automatic and manual calibration 
//        Added optional code in ReceiveMessage and Loop to handle FIFO vs processor decoding options.
...
#elif Subaru
   #include "Subaru.h"
#elif Mercedes_AMG_GT
   #include "Schrader_A9052102.h"
#endif
...
void heartbeatLed() {
  if (millis() - previousMillis >= interval) {
    ...
    digitalWrite(PIN_LED_HEART, !digitalRead(PIN_LED_HEART));
#endif
  }

#ifdef PIN_LED_DBG1
  // digitalWrite(PIN_LED_DBG1, !digitalRead(PIN_CC1101_CS));
  digitalWrite(DEBUGPIN, !digitalRead(CC1101_CS));
  // digitalWrite(PIN_BUZZER, !digitalRead(PIN_CC1101_CS));
  digitalWrite(PIN_BUZZER, !digitalRead(CC1101_CS));
#endif

#ifdef PIN_LED_DBG2
  // digitalWrite(PIN_LED_DBG2, digitalRead(PIN_CC1101_CD));
  digitalWrite(PIN_LED_DBG2, digitalRead(CDPin));
#endif

#ifdef PIN_LED_RX
  // digitalWrite(PIN_LED_RX, digitalRead(PIN_CC1101_RX));
  digitalWrite(LED_RX, digitalRead(RXPin));
#endif
}



void dumpCC1101config() {
  Serial.println(F("\nReg \tVal"));
  Serial.println(F("----\t----"));
  for(uint8_t i=0; i<=0x2E; i++) {
    uint8_t regVal = readReg(i,CC1101_CONFIG_REGISTER);
    // uint8_t regVal = readConfigReg(i);
...
void setup() {
  long intervalSerial = 5000;
  unsigned long previousMillisSerial = millis();

  Serial.begin(115200);
  while (!Serial && ((millis() - previousMillisSerial) < intervalSerial)) {
    heartbeatLed();
  }
  delay(500);

  // LEDs Setup and initialization
  pinMode(DEBUGPIN, OUTPUT);
  digitalWrite(DEBUGPIN, LOW);
  pinMode(PIN_LED_DBG2, OUTPUT);
  digitalWrite(PIN_LED_DBG2, LOW);
  ...
  digitalWrite(AUDIBLE_ALARM_PIN, LOW);
  ...
  // Display Backlight
#ifdef PIN_LCD_LITE
  pinMode(PIN_LCD_LITE, OUTPUT);
  digitalWrite(PIN_LCD_LITE, HIGH);
#endif
...
#ifdef USEFIFO
   Serial.println(F("Using CC1101 FIFO for detection"));
#else
   Serial.println(F("Using processor for timings and detection"));
#endif
  ...
  #elif Subaru
     Serial.println (F(" and Subaru TPMS sensor"));
  #elif Mercedes_AMG_GT
     Serial.println (F(" and Mercedes AMG GT TPMS sensor"));    
  #endif
  ...
  RegRead = readReg(CC1101_MCSM0,CC1101_CONFIG_REGISTER);
  if ((RegRead & 0x30) == 0)
  { 
     Serial.println("Autocal is off");
     AutoCal_Is_On = false;
  }
  else
  {
     Serial.println("Autocal is on");
     AutoCal_Is_On = true;
  }
...
//      if (CC1101_IsInRX() == false)
//      {//not in receive mode so reset it
//         RegRead = GetState();
//         Serial.print("MarcState=");
//         Serial.println(RegRead,HEX);
//         if ((RegRead == 8) || (RegRead == 5) || (RegRead == 12)|| (RegRead == 9))  //ignore if in calibration or settling
//         {
...
//         else
//         {
//            Serial.println("  Resetting RX mode");
//            setIdleState();
//            Flush_RX_FIFO(false);  
//            setRxState();   //will cause recalibration if in Autocal!     
//         }
//         Serial.println(GetState(),HEX);
//      }

      if (GetCarrierStatus() == true)
      {
         //Serial.println("CD Edge");
...
//         //Serial.print("MarcState=");
//         //Serial.println(RegRead,HEX);
//         if ((RegRead == 8) || (RegRead == 5) || (RegRead == 12) || (RegRead == 9))  //ignore if in calibration
//         {
//            //Serial.println("In Calibration");
//            delay(1);
//         }
