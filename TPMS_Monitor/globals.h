...
#define CAL_PERIOD_MS 180000 //3 * 60 * 1000  3 minutes
...
//   const int MAXTIMINGS = 255;
  ...
  /**** CC1101 ****/
  const int16_t CDPin = 26;     // PIN_CC1101_CD = 26;      //if wired, define 'USE_HW_CD' above, otherwise CD pin status is received over SPI
  const int16_t RXPin = 27;     // PIN_CC1101_RX = 27;      //must be an ext interrupt pin
  const int16_t CC1101_CS = 28; //PIN_CC1101_CS = 28;  // Define the Chip Select pin
  
  /**** LCD/SD CARD ****/
  // #define PIN_LCD_DC    6   // D/C
  // #define PIN_LCD_CS    7   // PIN_LCD_CS
  #define PIN_LCD_LITE  20  // Backlight
  // #define PIN_LCD_RESET 21  // Reset
  ...
  const int16_t MAXBITS = 1000;
  const int16_t MAXTIMINGS = 900;
  const int16_t SwNextPin = -1;
  const int16_t SwSelectPin = -1;
  const int16_t SwBackPin = -1;
#else
...
bool Audible_AlarmPin_Active = HIGH;
