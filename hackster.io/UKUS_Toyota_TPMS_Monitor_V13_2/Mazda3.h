...
//Mazda 3 Continental BDFL37140
    ...
    #define EXPECTEDBASEBITCOUNT 144  //Manchester coding so double number of 50us bits
...
void DecodeTPMS()
{
  ...
  status = RXBytes[4] ; // status bit and 0 filler
  ...
  realpressure = (realpressure * 0.198);
  if (realpressure < 0) 
     realpressure = 0.0;
  realtemp = (float)temp;
...
bool ValidateTimingsFast()
{
  ...
  StartDataIndex = ManchesterStartPos;
    ...
    {    
      #ifdef SHOWDEBUGINFO
         Serial.println(F("Mazda 3 header not found"));
      #endif
      StartPoint = 0;
      //return (false);   
    }
    else
    {
      StartPoint = ManchesterStartPos;
        ...
        {
           crcResult =  0xFF;  //ensure while comparison fails
        }          
