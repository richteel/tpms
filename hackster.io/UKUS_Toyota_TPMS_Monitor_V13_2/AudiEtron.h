...
//Audi E-tron
    ...
    #define EXPECTEDBASEBITCOUNT 80  
    #define EXPECTEDBYTECOUNT 11
    #define EXPECTEDBYTECOUNT_SHORT 8
    #define SYNCBITS 11
...
void DecodeTPMS(int16_t ByteCount)
{
  int16_t i;
  ...
  realpressure = (((realpressure) * 0.362) + .04);  //kpa to PSI: (2.45 /6.895 = 0.35534)
  if (realpressure < 0.0) 
     realpressure = 0.0;
  realtemp = (float)temp;
  ...
  switch(ManufacturerID)
  {
    ...
    case 0x80: Serial.print(F("Continental"));
               break;
    case 0x88: Serial.print(F("Audi(Triggered)"));
               break;
  ...
  Serial.print(F("   Tyre Pressure: (0x"));
...
bool ValidateTimingsFast()
{
  ...
  if (BaseBitCount >= EXPECTEDBASEBITCOUNT)
  {
      ...
      {
        #ifdef SHOWDEBUGINFO
           Serial.println(F("Audi header not found"));
        #endif
      ...
      {
         if (ByteCount == EXPECTEDBYTECOUNT_SHORT)
         {
            //debug purposes only!
            //PrintBytes(EXPECTEDBYTECOUNT);  
             crcResult = Compute_CRC8(7,0x2F, 0xAA);  //Checksum
            if (crcResult != RXBytes[7])
            {
               Serial.println(F("CRC Check2  failed"));
              #ifdef IGNORECHECKSUMERRORS
                DecodeTPMS(ByteCount);
                TPMS_Changed = false;  //don't update display for csum error
              #endif
         ...
         {
            //Serial.print(F("Manchester Byte Count = "));
