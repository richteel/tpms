...
//Toyota CHR (2020 PMV_C215 sensor)
    ...
    #define EXPECTEDBASEBITCOUNT 144  //Mnachester coding so double number of 50us bits
...
bool ValidateTimingsFast()
{
  ...
  {
     int Attempts = BaseBitCount - EXPECTEDBASEBITCOUNT+1;
     int StartPoint = 0;  //shift start point

     //keep shifting the data until a valid checksum or run out of data...
     do
     {
        //ByteCount = DecodeBitArrayFast(StartPoint, 0);
       
        StartDataIndex = StartPoint;
        ByteCount = BiPhaseSpaceDecode(StartPoint);
        ...
        {
           crcResult = RXBytes[8] - 1;  //ensure while comparison fails
        }          
